...
# многослойный перспетрон в библиотеке TensorFlow
# --------------------------------------------------
...
with tf.compat.v1.Session() as sess:
    ...
    x_ones = np.random.multivariate_normal(mean=np.array((0.75, 0.75)),
                                           cov=0.01 * np.eye(2),
                                           size=n_tests // 2)
    x_test = np.vstack([x_zeroes, x_ones])

    # расчет выходных значений обученной нейросети
    y_test = sess.run(y_predict, {x:x_test})
    # -----------------------------
...
xx=[p[0] for i,p in enumerate(x_test) if y_test[i]==0 ]
yy=[p[1] for i,p in enumerate(x_test) if y_test[i]==0 ]
plt.scatter(xx, yy, color='red', label='Predicted 0')

xx=[p[0] for i,p in enumerate(x_test) if y_test[i]==1 ]
yy=[p[1] for i,p in enumerate(x_test) if y_test[i]==1 ]
plt.scatter(xx, yy, color='green', label='Predicted 1')
