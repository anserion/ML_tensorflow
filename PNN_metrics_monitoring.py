# --------------------------------------------------
# оценка процесса увеличения точности (обучения) нейронной сети
# и минимизации функции потерь
# с визуализацией сразу двух процессов
# --------------------------------------------------
...
with tf.compat.v1.Session() as sess:
    ...
    for i in range(n_epochs):
        _,loss,predict=sess.run([train_op,f_loss,y_predict],
                        {x:x_train,y:y_train})
    ...
    # Построение визуализации функции потерь при обучении классификатора
    xx=[i for i in range(n_epochs) ]
    plt.scatter(xx,val_loss, color='blue', label='Train loss')
